...
function updateSettings(e) {
  ...
  socket.emit("settingsUpdate", {
    ...
    customWords: Array.from(
      new Set(
        document
          .querySelector("#customWords")
          .value.split("\n")
          .map((word) => word.trim())
          .filter((word) => word !== "")
      )
...
function showCanvasArea() {
  ...
  sketchpad.src =
    "https://cdn.jsdelivr.net/npm/responsive-sketchpad/dist/sketchpad.min.js";
...
if (searchParams.has("id")) {
  ...
  document.querySelector("#playGame").addEventListener("click", async () => {
    ...
    if (searchParams.has("id")) {
      ...
      }//${window.location.host}/?id=${searchParams.get("id")}`;
      putPlayer(my);
    }
  ...
  document.querySelector("#createRoom").addEventListener("click", async () => {
    ...
    socket.on("newPrivateRoom", (data) => {
      document.querySelector(
        "#gameLink"
      ).value = `${window.location.protocol}//${window.location.host}/?id=${data.gameID}`;
      putPlayer(my);
    });
  });
}
